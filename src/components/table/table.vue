<template>
    ...
    <div>
        <table>
            <thead>
                <tr>
                    <th v-for="(item, index) in columnsList" :key="'col' + index">
                        <div style="position: relative;" :class="{flex: item.hasSort}">
                            <div v-if="item.hasFilter" class="filter-area" @click="filterBy(item, index)">
                                <img class="filter" src="@/assets/icons/filter.png" />
                            </div>
                            <span>{{ item.colName }}</span>
                            <div v-if="item.hasSort" class="sort-area" @click="sortBy(item, index)">
            ...
            <tbody>
                <tr v-for="item in currentListData" :key="'table' + item.id">
                    <td v-for="(key, index) in columnsList" :key="'colInner' + index">
                        {{ item[key.field] }}
                    </td>
                </tr>
            </tbody>
        ...
        <!-- 分页组件 -->
        <pagination @current-change="handleCurrentChange" :size="limit" :currentPage="pageNum" :total="listData.length" ref="pagination"></pagination>
    </div>
</template>
...
export default {
    ...
    created() {
        ...
        this.currentListData = this.listData.slice(this.start, this.limit)
    },
    methods: {
        ...
        filterFun({field, selectArr}) {
            ...
            if (selectArr.length > 0) {
                ...
                this.recoverPagi()
                this.$refs.pagination.getPageList(this.listData.length)
            } else {
                this.showBackData = []
            }
        },
        ...
        handleCurrentChange(val) {
            ...
            this.currentListData = this.listData.slice(this.start, this.limit * val)
            this.pageNum = val
